...
#define DHTPin D6 //inisialisasi pin Din DHT
#define trigPin D8 //inisialisasi pin triger HC-SR04
#define echoPin D7 //inisialisasi pin echo HC-SR04
DHT dht(DHTPin, DHTTYPE); //maping konfig sensor DHT
#define lamp D0 //insialisasi pin OUT lampu
#define fan D1 //inisialisai pin OUT kipas
#define pump D2 //inisialisasi pin OUT pompa
...
int virtualSwitch;
//=====================================================//
char auth[] = "PtL-0P8a3iIA6YVF3IyOhAUZl_g3mCGL";
char ssid[] = "A71";
char pass[] = "12345677";
...
//const int trigPin = D8; //inisialisasi pin triger ultrasonik
//const int echoPin = D7; //inisialisasi pin echo ultrasonik
...
BLYNK_WRITE(V6)
{
 virtualSwitch = param.asInt(); // assigning incoming value from pin V6 to a
//variable
}
void clockDisplay() // sub-program "clockDisplay" utk menampilkan waktu
{
 int t_send = int(t);
 int h_send = int(h);
 String currentTime = String(hour()) + ":" + minute() + ":" + second(); //varibel
//currentTime menampung data JAM, MENIT, DETIK
 String currentDate = String(day()) + "-" + month() + "-" + year(); //varibel
//currentDate menampung data HARI, BULAN, TAHUN
...
void setup()
{
 ...
 timer.setInterval(1000L, clockDisplay); //set interval for sending value to
//BLYNK APP
}
void loop()
{
 h = dht.readHumidity(); //variable h reading Humdity value
 t = dht.readTemperature(); //vaiable t reading Temperature value
 ...
 timer.run(); //running timer
  while (!Blynk.connected()){ // re-connect with inet and BLYNK APP when
//sometimes get lose
